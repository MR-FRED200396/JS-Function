...
function t3() {
  ch3 = document.querySelector(".ch-3");
  return ch3.checked ? ch3.value : false;
}
...
function t4() {
  r4 = document.querySelectorAll(".r-4");
...
function t14() {
  let rezult = "";
  rezult = `rgb(${t13(0, 250)} ${t13(0, 250)} ${t13(0, 250)})`.trim();
  return rezult;
}
...
function t25(arr) {
  ...
  return rezult === arr.length ? true : false;
}
...
function t26() {
  let i26 = document.querySelector(".i-26").value.trim();
  let rezult = document.querySelector(".out-26");
  if (i26 !== "") {
    return (rezult.innerHTML = i26.toLowerCase());
  } else {
    return;
  }
}
...
function t27(v) {
  if (Object.values(obj27).includes(v)) {
