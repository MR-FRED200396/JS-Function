...
function t2(elem) {
  ...
  r2(elem);
  function r2(element) {
    isNaN(+element.textContent + s) ? true : (s += +element.textContent);
...
function t4(elem) {
  ...
  function r4(element) {
    Array.from(element.children).forEach((item) => {
      item.childNodes.length === 1 ? (item.style.background = "orange") : false;
      r4(item);
    });
  }
}
...
function t5(elem) {
  ...
  function r5(element) {
    ...
    if (innerElements.length === 0) {
      ...
      element.outerHTML = "<mark>" + innerText + "<mark>";
    }
